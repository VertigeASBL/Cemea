...
function exec_exportfiche() {
    ...
    if (_request('okconfirm')) {    /* **** Ecrire CSV ***** */
        ...
        case 1: //--- auteurs personne ---
            $trow = array(0=>'spip_auteurs.id_auteur','nom','bio','email','login','statut','maj');
            ...
            $k = 'personnes';
            $r_from = 'spip_auteurs, spip_auteurs_articles as A';
            $r_cond = 'statut<>\'5poubelle\' AND date_maj>=\''.$date_cond.'\'';
            $r_ordr = 'spip_auteurs.id_auteur';
            break;
        ...
        /*
        *   Didier: ont ajoute le filtre sur le type d'action.
        */
        if (_request('id_action_filtre')) $r_cond .= ' AND A.id_article='.sql_quote(_request('id_action_filtre'));

        //--- requete sql
        // echo $res = sql_get_select(implode(',', $trow), $r_from, $r_cond, null, $r_ordr);
...
//echo '</pre>';
    ...
    echo 'Les donn&eacute;es des fiches peuvent &ecirc;tre obtenues dans un fichier au format CSV.<br /><br />Vous pouvez enregistrer ce ¢ sur votre disque et/ou l\'ouvrir dans votre application tableur (Excel par exemple).',"\n";
    ...
    */
    $actions = sql_allfetsel('id_article, idact, titre', 'spip_articles', 'idact != \'\'', '', 'date_debut DESC');

    echo '<select name="id_action_filtre" id="actions">';
    echo '<option></option>';
