...
function exec_gestion_mass_pdf() {
	...
	if (session_get('statut') != '0minirezo') {
		...
		$inscrits = sql_allfetsel('a.id_auteur, email, nom, prenom', 'spip_auteurs_articles AS a INNER JOIN spip_auteurs AS b ON a.id_auteur = b.id_auteur', 'a.id_article='.sql_quote($id_article).' AND a.inscrit =\'Y\'');
