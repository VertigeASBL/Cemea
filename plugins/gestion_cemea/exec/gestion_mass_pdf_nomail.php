<?php
// set_time_limit(0);

if (!defined("_ECRIRE_INC_VERSION")) return;

function exec_gestion_mass_pdf_nomail() {

    include_spip('inc/presentation');
    include_spip('gestion_autorisation');

    include_spip('dompdf/dompdf_config.inc');
    include_spip('PDFMerger/PDFMerger');
    include_spip('fonctions_gestion_cemea');

    /* On vérifie que c'est bien un administrateur qui est connecté */
    if (session_get('statut') != '0minirezo') {
        include_spip('inc/minipres');
        echo minipres('Vous n\'avez pas les autorisations');
    }
    else {
        // Récupération des variables
        // Le squelette ou l'article qui sera utilisé pour créer le pdf.
        $modele = _request('modele');

        // Si on a un id_article on le récupère
        $id_article = _request('id_article');

        // On récupère les informations de l'action
        $action = sql_fetsel('titre, idact as reference', 'spip_articles', 'id_article='.sql_quote($id_article));

        /* On va boucler sur tout les inscrits de l'article */
        $inscrits = sql_allfetsel('a.id_auteur, email, nom, prenom', 'spip_auteurs_articles AS a INNER JOIN spip_auteurs AS b ON a.id_auteur = b.id_auteur', 'a.id_article='.sql_quote($id_article).' AND a.inscrit =\'Y\' AND b.send_email ='.sql_quote(''));

        // On vérifie qu'il y a bien des gens qui ne veulent pas de mail
        if (count($inscrits) > 0) {

            // On va avoir besoi de fusionner tout les PDF
            $merge = new PDFMerger;

            // On boucle sur tout les inscrits, pour créer et envoyer les PDF 
            foreach ($inscrits as $key => $value) {

                $html = recuperer_fond(
                    'squelettes/generique', 
                    array(
                        'id_pdf' => _request('modele'),
                        'id_activite' => $id_article,
                        'id_personne' => $value['id_auteur']
                        ), 
                    array('ajax' => false)
                    );
                
                // On crée le nom du fichier
                $filename = $id_article.$value['id_auteur'].'.tmp.pdf';

                // Initialisation de la lib et création du PDF
                $dompdf = new DOMPDF();

                // On charge le HTML
                $dompdf->load_html($html);

                // Render !
                $dompdf->render();

                // Récupération du pdf sous forme de flux
                $file = $dompdf->output();

                // On créer un fichier avec notre PDF
                file_put_contents(sous_repertoire(_DIR_CACHE, 'gestion').$filename.'.pdf', $file);

                // Une fois le PDF créer on le stage pour être fusionner en un seul PDF.
                $merge->addPDF(sous_repertoire(_DIR_CACHE, 'gestion').$filename.'.pdf');
            }

            // Un fois que tout est stager on fusionne les PDF en un seul.
            // Fuuuuusion !
            $file = $merge->merge('download', $action['reference'].'_no_mail.pdf');
        }
        else {
            $commencer_page = charger_fonction('commencer_page','inc');
            echo $commencer_page(_T('gestion:tableau_de_bord'));

            echo debut_cadre_relief();
            echo _T('gestion:no_no_mail');
            echo fin_cadre_relief();

            echo fin_page();
        }
    }  
}
?>