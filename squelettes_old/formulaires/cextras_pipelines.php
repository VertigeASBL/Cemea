...
function cextras_editer_contenu_objet($flux){
	...
	if ($flux['args']['type']=='article') {
		...
		case 'editer_auteur': //--- cote admin
			$taff = array('idper','typepart','archive_per','codecourtoisie','prenom','fonction','nom_court_institution','nom_long_institution','description_institution','date_naissance','adresse','adresse_no','codepostal','localite','tel1','tel2','gsm1','gsm2','fax1','fax2','email2','date_creation','date_maj','statut_form_cemea','statut_anim_cemea','statut_ep','statut_sj','personne_reference','centre_reference','envoi_diffusion','date_debut_diffusion','date_fin_diffusion','diffusion','ndiffusion','adherent');
			break;
		...
		case 'inscrire_auteur': //--- cote public : modifier un auteur, inscrire a une action
			...
			$taff = array('typepart','codecourtoisie','prenom','fonction','nom_court_institution','nom_long_institution','date_naissance','adresse','adresse_no','codepostal','localite','tel1','gsm1','fax1','diffusion','ndiffusion',
                            'alimentation'
                            );
		...
		case 'adherer_auteur': //--- adherent
			$taff = array('codecourtoisie','prenom','date_naissance','adresse','adresse_no','codepostal','localite','tel1','gsm1','diffusion');
			break;
		}
	}
	else if ($flux['args']['type']=='auteurs_article') {
		//--- Suivi des inscriptions ---
		$taff = array('statutsuivi','date_suivi','heure_suivi','alimentation','remarques_inscription','sante_comportement','ecole','places_voitures','brevet_animateur');
	}
	...
	if ($extras = cextras_get_extras_match($flux['args']['type'])) {
		...
		if (isset($flux['args']['prefixe_champs_extras']) and $prefixe = $flux['args']['prefixe_champs_extras']) {
			...
			$prefixe = '';
                ...
                if($id_action=_request("idaction")) {
...
//                                    print_r($contexte);
                ...
                if($other_extras && (($id_action=_request("idaction")) || ($id_meta_action_rub=_request("idrubrique")))) {
                    $add_fields=FALSE;
...
//                        if($add_fields) { //Removed condition because alimentation is always required
                            //Get values from DB
