...
/**
 * Retourne la description (classe ChampExtra) d'un champ extra d'un objet donnÃ©.
 *
...
function cextras_editer_contenu_objet($flux){
	...
	if ($flux['args']['type']=='article') {
		...
		case 'editer_auteur': //--- côté admin
			...
			break;
		case 'select_auteur': //--- sélection des destinataires
			$taff = array('diffusion');
			break;
		case 'spip_listes_inscription': case 'gestion_abonnement': //--- abonnement
			$taff = array('diffusion');
			break;
		case 'inscrire_auteur': //--- côté public : modifier un auteur, inscrire à une action
			$taff = array('typepart','codecourtoisie','prenom','fonction','nom_court_institution','nom_long_institution','description_institution','date_naissance','adresse','codepostal','localite','tel1','tel2','gsm1','gsm2','fax1','fax2','email2','diffusion','ndiffusion');
			break;
		case 'adherer_auteur': //--- adhérent
			$taff = array('codecourtoisie','prenom','date_naissance','adresse','codepostal','localite','tel1','gsm1','diffusion');
			break;
		}
/*	$taff = array('idper','archive_per','date_creation','date_maj','statut_form_cemea','statut_anim_cemea','statut_ep','statut_sj','personne_reference','centre_reference','envoi_diffusion','date_debut_diffusion','date_fin_diffusion','adherent'); */
	}
	else if ($flux['args']['type']=='auteurs_article') {
		//--- Suivi des inscriptions ---
		$taff = array('statutsuivi','date_suivi','heure_suivi');
	}
// echo '<pre>'; print_r($flux['args']['contexte']); echo '</pre><hr />',"\n";
	...
	if ($extras = cextras_get_extras_match($flux['args']['type'])) {
		...
		// Il peut arriver qu'un prefixe soit appliquÃ© sur les noms de champs de formulaire
		...
		// La contrainte est que le formulaire inserÃ© doit appeler le pipeline 'editer_contenu_objet'
		// en lui indiquant quel est le prefixe utilisÃ© d'une part, et d'autre part
		// il faut qu'il s'occupe lui mÃªme d'ajouter les donnÃ©es via
		...
		foreach ($extras as $c) {
				...
				'contexte' => $flux['args']['contexte'])))
			{
				...
				// par exemple pour afficher et completer un objet diffÃ©rent dans
		...
		foreach($extras as $c) {
				...
				'contexte' => $contexte)))
			{
				...
				($f = 'saisies-vues/'.$c->type).'.html')){
					...
					// ajouter les listes d'Ã©lÃ©ments possibles
...
function cextras_formulaire_verifier($flux){
	...
	if (strncmp($form, 'editer_', 7) === 0) {
		...
		if ($extras = cextras_get_extras_match($type)) {
			...
			// Il peut arriver qu'un prefixe soit appliquÃ© sur les noms de champs de formulaire
			// La contrainte est que le formulaire inserÃ© doit appeler le pipeline 'formulaire_verifier'
			// avec le bon type d'objet (en indiquant le prefixe) et concatÃ©ner ainsi les rÃ©sultats
